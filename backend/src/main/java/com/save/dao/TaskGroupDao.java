package com.save.dao;

import com.save.entity.TaskGroup;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * (TaskGroup)表数据库访问层
 *
 * @author makejava
 * @since 2021-01-05 15:39:04
 */
@Mapper
public interface TaskGroupDao {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    TaskGroup queryById(Integer id);

    /**
     * 查询指定行数据
     *
     * @param offset 查询起始位置
     * @param limit  查询条数
     * @return 对象列表
     */
    List<TaskGroup> queryAllByLimit(@Param("offset") int offset, @Param("limit") int limit);


    /**
     * 通过实体作为筛选条件查询
     *
     * @param taskGroup 实例对象
     * @return 对象列表
     */
    List<TaskGroup> queryAll(TaskGroup taskGroup);

    /**
     * 新增数据
     *
     * @param taskGroup 实例对象
     * @return 影响行数
     */
    int insert(TaskGroup taskGroup);

    /**
     * 批量新增数据（MyBatis原生foreach方法）
     *
     * @param entities List<TaskGroup> 实例对象列表
     * @return 影响行数
     */
    int insertBatch(@Param("entities") List<TaskGroup> entities);

    /**
     * 批量新增或按主键更新数据（MyBatis原生foreach方法）
     *
     * @param entities List<TaskGroup> 实例对象列表
     * @return 影响行数
     */
    int insertOrUpdateBatch(@Param("entities") List<TaskGroup> entities);

    /**
     * 修改数据
     *
     * @param taskGroup 实例对象
     * @return 影响行数
     */
    int update(TaskGroup taskGroup);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(Integer id);

}